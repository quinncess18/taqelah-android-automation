...
> **Transitional-state auto-wait:** `getPermissionStatus()` in `PermissionsPage.js` auto-waits through transitional states like Location's `"Getting location..."` state (polling every 500ms up to 15s) until the status settles to a final value like `"Granted"`. This prevents flaky assertions after re-navigation when the permission is still resolving.
